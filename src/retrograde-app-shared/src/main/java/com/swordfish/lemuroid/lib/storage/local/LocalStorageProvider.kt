...
class LocalStorageProvider(
    ...
    override fun listBaseStorageFiles(): Flow<List<BaseStorageFile>> {
        ...
        return flow {
            paths.forEach { path ->
                val folder = when {
                    path.startsWith("file://") -> File(Uri.parse(path).path ?: return@forEach)
    ...
    private fun walkDirectory(rootDirectory: File): Flow<List<BaseStorageFile>> =
        flow {
            Timber.d("DEBUG: walkDirectory called with: ${rootDirectory.absolutePath}, exists=${rootDirectory.exists()}, isDir=${rootDirectory.isDirectory}")
            ...
            while (directories.isNotEmpty()) {
                ...
                Timber.d("DEBUG: Listing directory: ${directory.absolutePath}, result=${files?.size ?: "null"}")
        ...
        return try {
            ...
            if (uri.scheme == "file") {
                ...
                if (file.exists()) {
                     ...
                     Timber.i("Deleted local file: ${file.absolutePath}, success=$deleted")
                     deleted
                } else {
                    Timber.w("File to delete not found: ${file.absolutePath}")
                    false
                }
            } else {
                Timber.w("Unsupported URI scheme for deletion: ${uri.scheme}")
                false
            }
        } catch (e: Exception) {
            Timber.e(e, "Error deleting local file")
            false
        }
    }
