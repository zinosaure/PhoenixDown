...
class StorageAccessFrameworkProvider(
    ...
    override fun listBaseStorageFiles(): Flow<List<BaseStorageFile>> {
        ...
        return flow {
            uris.forEach { uriStr ->
                traverseDirectoryEntries(Uri.parse(uriStr)).collect { emit(it) }
            }
        }
    }
    ...
    override suspend fun delete(game: Game): Boolean {
        return try {
            ...
            if (document != null && document.exists()) {
                ...
                Timber.i("Deleted SAF file: ${game.fileName}, success=$deleted")
                deleted
            } else {
                Timber.w("SAF File to delete not found: ${game.fileName}")
                false
            }
        } catch (e: Exception) {
            Timber.e(e, "Error deleting SAF file")
            false
        }
    }
